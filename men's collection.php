...
<body>
  ...
  <a class="navbar-brand logo-size" href="home.php"><img src="png/Myntra.png" style="width: 70px;
        ...
        <form action="men's collection.php" method="post" enctype="multipart/form-data" style="margin-left: 5%">
            <div class="input-group">
              ...
              <div class="input-group-append">
                <button class="btn btn-outline-secondary" type="submit" value="Samecolor" name="submit1">Same Color</button>
                <button class="btn btn-outline-secondary" type="submit" value="Sameacc" name="submit2">Matching Accessories</button>
              </div>
            </div>
        </form>
...
if(isset($_POST["submit1"])) {
    ...
    foreach ($q as $value)
    {
      if (substr($value, 0,4)=='wacc' or substr($value, 0,4)=='wclo') {
        continue;
      }
              $i = str_replace("\\", "/", $value);
              $i = 'img_fashion/'.$i;
              $queri = mysqli_query($conn,"SELECT * FROM mclothes WHERE img_path = '$i'");
    ...
    foreach ($q as $value)
    {
      if (substr($value, 0,4)=='wacc' or substr($value, 0,4)=='wclo' or substr($value, 0,4)=='mclo') {
        continue;
      }
              $i = str_replace("\\", "/", $value);
              $i = 'img_fashion/'.$i;
              $queri = mysqli_query($conn,"SELECT * FROM macc WHERE img_path = '$i'");
              $row = mysqli_fetch_assoc($queri);
              if ($row['img_path']=='') {
                continue;
              }
...
<div class="container mb-5 mt-5" id="pro">
  <?php
    $query = mysqli_query($conn,"SELECT * FROM mclothes");
